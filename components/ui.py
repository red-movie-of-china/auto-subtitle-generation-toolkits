...
def Single_Operation(media_path:str,media_name:str)->int:
    """
        ...
        返回值:
            ...
            exception: 失败
    """
    print('try open ',media_name,' in folder ',media_path)
    while Locate_Status() !=1:
        Restart_Client()
        into_Main_Window()
    #### 定义一些Position
    ...
    def srt_identify():
        ...
        if __name__ == "__main__":
            ...
            path=media_path
        print('srt==',path+os.sep+'.'.join(media_name.split(".")[:-1])+".srt")
        with open(path+os.sep+'.'.join(media_name.split(".")[:-1])+".srt", 'w', encoding='utf-8') as f:  f.write(simple_srt.tracks_to_srt_string(tracks))
...
def Multi_Video_Process(video_path:str=os.path.abspath(CONFIG["Video_Path"])):
    ...
    media_list = [fn for fn in os.listdir(video_path) if any(fn.endswith(format) for format in ['.aac','.m4a','.mp3'])]
    for item in media_list:
        videoext='.'+item.split('.')[-1]
        videoname=item.split('.')[0]
        aac_name = videoname+".aac"
        os.system(f"echo Start Processing {aac_name}")
        if os.path.exists(video_path+os.sep+aac_name):
            if not os.path.exists(video_path+os.sep+videoname+'.srt'):
                result = Single_Operation(media_path=video_path,media_name=aac_name)
                if result == 0: os.system(f"echo {aac_name} Success")
                Restart_Client(True)
                if CONFIG["webhook"] : requests.post(CONFIG["webhook_url"],headers={"User-Agent":"JySrtParser"},json={"content":f"{aac_name} Success","time":time.time()})
