...
def test_exe(name):
    ...
    for p in os.environ.get("PATH", "").split(os.pathsep):
        ...
        if os.access(exe, os.X_OK):
            return True
def split_audiofile(fname,l=7200,remove=False):
...
def transcode_chapter(fname, dest_dir, op, ext, digits, i, chap, start, end, activation_bytes=None):
    ...
    try:
        ...
        out_file = os.path.join(dest_dir, "{i:0{digits}}{ext}".format(i=i, digits=digits, chap=chap, ext=ext))
